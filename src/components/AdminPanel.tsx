...
const AdminPanel: React.FC = () => {
  ...
  const [editingTweetUrl, setEditingTweetUrl] = useState(false);
  ...
  const loadTask3Config = async () => {
    try {
      ...
      if (data) {
        ...
        setNewTweetUrl(data.tweet_url);
      }
    } catch (error) {
      console.error('Error loading Task 3 config:', error);
    }
  };

  const handleUpdateTweetUrl = async () => {
    if (!newTweetUrl.trim()) {
    ...
    try {
      ...
      setEditingTweetUrl(false);
      alert('Tweet URL updated successfully!');
    } catch (error: any) {
      console.error('Error updating tweet URL:', error);
      alert('Failed to update tweet URL: ' + error.message);
    }
  };

  const handleResetTask3 = async () => {
    if (!resetReason.trim()) {
      alert('Please provide a reason for resetting Task 3');
      return;
    }
    ...
    try {
      ...
      const { data: tasks, error: countError } = await supabase
        .from('credit_tasks')
        .select('id')
        .eq('task_type', 'twitter_like_repost')
        .in('status', ['submitted', 'pending']);

      if (countError) throw countError;

      const submissionsCount = tasks?.length || 0;
      ...
      alert(`Task 3 reset successfully! ${submissionsCount} submission(s) cleared.`);
    } catch (error: any) {
      console.error('Error resetting Task 3:', error);
      alert('Failed to reset Task 3: ' + error.message);
    }
  };
    ...
    <div className="min-h-screen bg-gray-50">
                    ...
                    <tr key={task.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center gap-2">
                          ...
                          ) : task.task_type === 'twitter_like_repost' ? (
                            <>
                              <ExternalLink className="h-4 w-4 text-purple-500" />
                              <span className="text-sm">Twitter Like & Repost</span>
                            </>
                          ) : (
                            <>
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              <div className="flex items-center justify-between mb-6">
                <div>
                  <h3 className="text-lg font-semibold text-gray-900">Task 3 Configuration</h3>
                  <p className="text-sm text-gray-600 mt-1">Configure the Like & Repost Tweet task</p>
                </div>
              </div>
              ...
              <div className="space-y-6">
                <div className="border border-gray-200 rounded-lg p-4">
                  <div className="flex items-start justify-between mb-4">
                    <div>
                      <h4 className="font-medium text-gray-900">Tweet URL</h4>
                      <p className="text-sm text-gray-500 mt-1">The tweet users need to like and repost</p>
                    </div>
                    {!editingTweetUrl && (
                      <button
                        onClick={() => setEditingTweetUrl(true)}
                ...
                <div className="border border-red-200 rounded-lg p-4 bg-red-50">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      <h4 className="font-medium text-red-900">Reset Task 3 Submissions</h4>
                      <p className="text-sm text-red-700 mt-1">
                        This will clear all pending and submitted Task 3 submissions. Approved tasks will NOT be affected.
                        Users will be able to submit again for the new tweet.
                      </p>
                      <button
      ...
      {showResetModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-2xl max-w-md w-full">
            <div className="flex items-center justify-between p-6 border-b border-gray-200">
              <h3 className="text-lg font-semibold text-gray-900">Reset Task 3</h3>
              <button
            ...
            <div className="p-6 space-y-4">
              <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                <div className="flex items-start gap-2">
                  ...
                  <div className="text-sm text-yellow-800">
                    ...
                    <p>This will delete all pending and submitted Task 3 submissions. Approved submissions will NOT be affected.</p>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-transparent"
                  rows={3}
                  placeholder="Enter reason for resetting Task 3..."
                />
              </div>
            </div>
